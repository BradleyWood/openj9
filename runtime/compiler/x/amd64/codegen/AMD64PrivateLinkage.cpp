...
static uint8_t *flushArgument(
   ...
   cursor = TR::InstOpCode(op).binary(cursor, OMR::X86::Default, 0, cg);
...
static int32_t flushArgumentSize(
      ...
      int32_t offset)
   {
   int32_t size = TR::InstOpCode(op).length(OMR::X86::Default, 0, TR::comp()->cg()) + 1;   // length including ModRM + 1 SIB
