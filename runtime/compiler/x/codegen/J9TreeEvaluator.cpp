...
static TR::Register* inlineStringHashCode(TR::Node* node, bool isCompressed, TR::CodeGenerator* cg)
   {
   TR::VectorLength vl = cg->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_AVX2) ? TR::VectorLength256 : TR::VectorLength128;
   uint32_t unrollCount = 4;

   static char *vlOverride = feGetEnv("TR_setStringHashCodeVL");

   if (vlOverride)
      {
      uint32_t length = atoi(vlOverride);
