...
TR::Register *
J9::X86::TreeEvaluator::vectorizedHashCodeReductionHelper(TR::Node* node, TR::Register **vectorRegisters, int32_t numVectors, TR::Register *tmpVectorReg, TR::Register *result, TR::VectorLength vl, TR::DataType dt, TR::CodeGenerator* cg)
   {
   TR::InstOpCode opcode = TR::InstOpCode::PADDDRegReg;
   TR::Register *vectorReg = vectorRegisters[0];
   ...
   for (int32_t i = 1; i < numVectors; i++)
      ...
      generateRegRegInstruction(TR::InstOpCode::PADDDRegReg, node, vectorReg, vectorRegisters[i], cg, opcodeEncoding);
      }
...
// signed and unsigned integer elements upto 32-bits in size, vector lengths from 128-bit up to 512-bit and up to 4x
// loop unrolling.
//
...
J9::X86::TreeEvaluator::vectorizedHashCodeLoopHelper(TR::Node *node,
                                                     ...
                                                     TR::CodeGenerator *cg)
   {
   OMR::X86::Encoding vectorEncodingMethods[3] = { OMR::X86::Default, OMR::X86::VEX_L256, OMR::X86::EVEX_L512 };
   int32_t vectorSizes[3] = { 4, 8, 16};
   int32_t shift = dt - TR::Int8; /* i8 -> 0, i16 -> 2, i32 -> 2 */
   ...
   TR::Register *hashXMM[]          = { cg->allocateRegister(TR_VRF), cg->allocateRegister(TR_VRF), cg->allocateRegister(TR_VRF), cg->allocateRegister(TR_VRF) };
   TR::Register *multiplier31PowN[] = { cg->allocateRegister(TR_VRF), cg->allocateRegister(TR_VRF), cg->allocateRegister(TR_VRF), cg->allocateRegister(TR_VRF) };

   deps->addPostCondition(tmp, TR::RealRegister::NoReg, cg);
   deps->addPostCondition(tmpXMM, TR::RealRegister::NoReg, cg);
   deps->addPostCondition(multiplierXMM, TR::RealRegister::NoReg, cg);

   for (int32_t i = 0; i < 4; i++) deps->addPostCondition(multiplier31PowN[i], TR::RealRegister::NoReg, cg);
   for (int32_t i = 0; i < 4; i++) deps->addPostCondition(hashXMM[i], TR::RealRegister::NoReg, cg);
   ...
   generateRegRegInstruction(TR::InstOpCode::MOVDRegReg4, node, hashXMM[0], initialHash, cg);
   for (int32_t i = 1; i < unrollCount; i++)
      generateRegRegInstruction(TR::InstOpCode::PXORRegReg, node, hashXMM[i], hashXMM[i], cg, vectorEncoding);
   ...
   generateRegMemInstruction(TR::InstOpCode::MOVDQURegMem, node, multiplierXMM, generateX86MemoryReference(cg->findOrCreateConstantDataSnippet(node, multiplier31PowNData, vectorSizeElements * sizeof(int32_t)), cg), cg, vectorEncoding);
   ...
   for (int32_t i = 0; i < unrollCount; i++)
      ...
      //
      // Given an unrolling factor (x) of 4, and a maximum vector length of 128-bits we will process upto 16 elements
      // per iteration of the main loop. This means the multiplication vectors are as follows:
      //
      //   multiplier31PowN[0] = (31^15, 31^14, 31^13, 31^12),
      //   multiplier31PowN[1] = (31^11, 31^10, 31^9, 31^8),
      //   multiplier31PowN[2] = (31^7, 31^6, 31^5, 31^4),
      //   multiplier31PowN[3] = (31^3, 31^2, 31^1, 31^0),
      //
      TR::Register *multiplier31PowN_i = multiplier31PowN[i];
   ...
   for (int32_t i = 0; i < unrollCount; i++)
      ...
      generateRegMemInstruction(loadOpcode, node, tmpXMM, mr, cg, vectorEncoding);

      // tmpXMM = tmpXMM * multiplierXMM
      generateRegRegInstruction(TR::InstOpCode::PMULLDRegReg, node, tmpXMM, multiplier31PowN[i], cg, vectorEncoding);
      // hashXMM = ( hashXMM * {31^vl, ..., 31^vl} ) + tmpXMM
      generateRegRegInstruction(TR::InstOpCode::PMULLDRegReg, node, hashXMM[i], multiplierXMM, cg, vectorEncoding);
      generateRegRegInstruction(TR::InstOpCode::PADDDRegReg, node, hashXMM[i], tmpXMM, cg, vectorEncoding);
      }
   }
   ...
   vectorizedHashCodeReductionHelper(node, hashXMM, unrollCount, tmpXMM, result, vl, dt, cg);
   generateLabelInstruction(TR::InstOpCode::label, node, endLabel, deps, cg);
   ...
   for (int32_t i = 0; i < 4; i++) cg->stopUsingRegister(multiplier31PowN[i]);
   for (int32_t i = 0; i < 4; i++) cg->stopUsingRegister(hashXMM[i]);
...
//
// This helper generates the following code to implement vectorized hashCode
//
//   1. Generate the main vectorized loop unrolled by a factor of 4, at highest available vector length
//   2. Generate a second vectorized loop, not unrolled, using 128-bit vectors
//   3. Process residue sequentially
//     a. for (; index < length; index++) { hash = 31 * hash + arr[index]; }
//
// This code relies on two helpers to generate the vectorized loop and to reduce the vectorized result into the scalar
// hashcode value.
//
//   1. vectorizedHashCodeLoopHelper(...)      -> Generates the vectorized loop code for given type and vector size
//   2. vectorizedHashCodeReductionHelper(...) -> Generates code to reduce x vectors by adding all elements together
//
// Future works should investigate the appropriate amount of unrolling based on block hotness, array length, and
// cache implications of unrolling. This inline intrinsic could pose significant pressure on the code-cache given
// the large expected size of the generated code with unrolling. This may be especially apparent if the vectorized
// hashCode algorithm is inlined multiple times in the same method.
//
TR::Register *
J9::X86::TreeEvaluator::vectorizedHashCodeHelper(TR::Node *node, TR::DataType dt, TR::Node *nodeHash, bool isSigned,
   ...
   TR::Register *initHash = cg->intClobberEvaluate(nodeHash);
   ...
   generateRegRegInstruction(TR::InstOpCode::MOVRegReg(false), node, initHash, result, cg);
   ...
   generateRegRegImmInstruction(TR::InstOpCode::IMULRegRegImm4(), node, result, result, 31, cg);

   TR::InstOpCode::Mnemonic signedLoadOpcode[3]   = { TR::InstOpCode::MOVSXReg4Mem1, TR::InstOpCode::MOVSXReg4Mem2, TR::InstOpCode::L4RegMem };
   TR::InstOpCode::Mnemonic unsignedLoadOpcode[3] = { TR::InstOpCode::L1RegMem, TR::InstOpCode::L2RegMem, TR::InstOpCode::L4RegMem };
